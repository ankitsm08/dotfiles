...
config = {
  ...
  open_subtitles_api_key = "b0rd16N0bp7DETMpO4pYZwIqmQkZbYQr", -- gitleaks:allow
  ...
  types = {
    ...
    media = comma_split(
      options.video_types .. "," .. options.audio_types .. "," .. options.image_types .. "," .. options.playlist_types
    ),
...
mp.observe_property("demuxer-cache-state", "native", function(prop, cache_state)
  ...
  if
    not (state.duration and (#cached_ranges > 0 or state.cache == "yes" or (state.cache == "auto" and state.is_stream)))
  then
    ...
    serializer = function(audio_device_list, current_device)
      ...
      for _, device in ipairs(audio_device_list) do
        if device.name == "auto" or string.match(device.name, "^" .. ao) then
          ...
          items[#items + 1] = {
            title = device.description:sub(1, 7) == "Default" and t("Default %s", device.description:sub(9))
