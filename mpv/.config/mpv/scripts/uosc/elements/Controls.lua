...
function Controls:init_options()
  ...
  for i, item in ipairs(items) do
    ...
    config = shorthands[config_tooltip[1]] and split(shorthands[config_tooltip[1]], " *%? *")[1] or config_tooltip[1]
    ...
    elseif kind == "command" then
      if #params ~= 2 then
        mp.error(
          string.format("command button needs 2 parameters, %d received: %s", #params, table.concat(params, "/"))
        )
      else
    ...
    elseif kind == "cycle" then
      if #params ~= 3 then
        mp.error(string.format("cycle button needs 3 parameters, %d received: %s", #params, table.concat(params, "/")))
      else
    ...
    elseif kind == "button" then
      if #params ~= 1 then
        mp.error(string.format("managed button needs 1 parameter, %d received: %s", #params, table.concat(params, "/")))
      else
    ...
    if control.element then
      for _, prop in ipairs(disposition_props) do
        control.element:observe_mp_property(prop, function()
          self:reflow()
        end)
      end
    end
