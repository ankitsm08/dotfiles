...
local function load_files(dtitle, destination, audio, wait)
  ...
  local loadOpts = audio
    .. 'force-media-title="'
    .. dtitle
    .. '",demuxer-max-back-bytes=1MiB,demuxer-max-bytes=3MiB,ytdl=no,script-opt=ytdl_preload-id='
    .. dvID
...
local function listener(event)
  if
    not caught
    and event.prefix == mp.get_script_name()
    and string.find(event.text, "%[download%] Destination: ")
    and string.find(destination, string.gsub(cachePath, "~/", ""))
  then
...
local function DL()
  ...
  if
    (enabledOpt and enabledOpt == "false")
    or (tonumber(mp.get_property("playlist-count")) == 1)
    or (not mp.get_property("playlist/" .. index + 1 .. "/filename"):find("://", 0, false))
  then
    return
  end
